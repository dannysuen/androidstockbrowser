...
/**
 * Extends the schema of a ContentProvider to include the _sync_state table and implements
 * query/insert/update/delete to access that table using the authority "syncstate". This can be used to store
 * the sync state for a set of accounts.
 */
public class SyncStateContentProviderHelper {
	...
	public void createDatabase(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE " + SYNC_STATE_META_TABLE + " (" + SYNC_STATE_META_VERSION_COLUMN + " " +
				"INTEGER);");
	...
	public void onDatabaseOpened(SQLiteDatabase db) {
		long version = DatabaseUtils.longForQuery(db,
				"SELECT " + SYNC_STATE_META_VERSION_COLUMN + " FROM " + SYNC_STATE_META_TABLE, null);
