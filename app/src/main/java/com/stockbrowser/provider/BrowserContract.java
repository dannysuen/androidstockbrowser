...
public class BrowserContract {
	 ...
	 * "syncToNetwork" parameter is set to false when calling {@link ContentResolver#notifyChange(Uri,
	...
	interface SyncColumns extends BaseSyncColumns {
		 ...
		 * owning account. 指示标志{版本}已经改变，该行需要其拥有帐户同步。 1 表示版本发生变化了 0 表示版本相同了 <P>Type: INTEGER (boolean)</P>
		 */
	...
	interface CommonColumns {
		...
		/**
		 * The URL of the bookmark. <P>Type: TEXT (URL)</P>
		 */
		public static final String URL = "url";

		/**
		 * The user visible title of the bookmark. <P>Type: TEXT</P>
		 */
	...
	public static final class Bookmarks implements CommonColumns, ImageColumns, SyncColumns {
		/**
		 * The content:// style URI for this table 这个表格的URI样式
		 */
		public static final Uri CONTENT_URI = Uri.withAppendedPath(AUTHORITY_URI, "bookmarks");
		/**
		 * The content:// style URI for the default folder 默认文件夹的URI样式
		 */
		public static final Uri CONTENT_URI_DEFAULT_FOLDER =
				Uri.withAppendedPath(CONTENT_URI, "folder");
		/**
		 * Query parameter used to specify an account name 通过这个参数指定某个账户
		 */
		public static final String PARAM_ACCOUNT_NAME = "acct_name";
		/**
		 * Query parameter used to specify an account type 通过这个参数指定某个账户类型
		 */
		 ...
		 * and
		 * haven't been synced yet. 使用查询参数，如果你想看到已删除的书签仍然          *周围的设备上，并没有同步
		 *
		 * @see #IS_DELETED
		 */
		 ...
		 * indicates a bookmark. 1表示文件夹 0表示一条网页数据 -1表示自动提示 <P>Type: INTEGER (boolean)</P>
		 */
		public static final String IS_FOLDER = "folder";
		/**
		 * The ID of the parent folder. ID 0 is the root folder. 父文件夹的_ID    如果是0表示根目录 <P>Type: INTEGER
		 * (reference to item in the same table)</P>
		 */
		public static final String PARENT = "parent";
		/**
		 * The source ID for an item's parent. Read-only. 父对象的源ID 是只读的
		 *
		 * @see #PARENT
		 */
		public static final String PARENT_SOURCE_ID = "parent_source";
		/**
		 * The position of the bookmark in relation to it's siblings that share the same 相对于它的兄弟姐妹共享相同的位置中的书签
		 * {@link #PARENT}. May be negative. <P>Type: INTEGER</P>
		 */
		public static final String POSITION = "position";
		/**
		 * The item that the bookmark should be inserted after. 该项目后应插入书签。可能是负面的。 May be negative. <P>Type:
		 * INTEGER</P>
		 */
		 ...
		 * query.
		 * 0表示没删除   1表示删除掉 <p>Type: INTEGER (non-zero if the item has been deleted, zero if it hasn't)
		 *
		 ...
		 *
		 * @param folderId the ID of the folder to point to 创建一个文件夹的URI
		 */
