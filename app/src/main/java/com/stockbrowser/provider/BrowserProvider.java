...
public class BrowserProvider extends ContentProvider {
	...
	/**
	 * Strips the provided url of preceding "http://" and any trailing "/". Does not strip "https://". If the
	 * provided string cannot be stripped, the original string is returned. <p/> TODO: Put this in TextUtils
	 * to be used by other packages doing something similar.
	 *
	 * @param url a url to strip, like "http://www.google.com/"
	 * @return a stripped url like "www.google.com", or the original string if it could not be stripped
	 */
	...
	static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				for (int i = 0; i < size; i = i + 2) {
					CharSequence bookmarkDestination = replaceSystemPropertyInString(mContext, bookmarks[i +
							1]);
					db.execSQL("INSERT INTO bookmarks (title, url, visits, " +
							"date, created, bookmark)" + " VALUES('" +
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion < 22) {
				db.execSQL("DELETE FROM bookmarks WHERE (bookmark = 0 AND url LIKE \"%.google" +
						".%client=ms-%\")");
				removeGears();
			}
			if (oldVersion < 23) {
	...
	private class MySuggestionCursor extends AbstractCursor {
		...
		/**
		 * Provides the title (text line 1) for a browser suggestion, which should be the webpage title. If
		 * the webpage title is empty, returns the stripped url instead.
		 *
		...
		/**
		 * Provides the subtitle (text line 2) for a browser suggestion, which should be the webpage url. If
		 * the webpage title is empty, then the url should go in the title instead, and the subtitle
		 * should be
		 * empty, so this would return null.
		 *
