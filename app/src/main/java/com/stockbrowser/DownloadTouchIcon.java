...
class DownloadTouchIcon extends AsyncTask<String, Void, Void> {
	...
	/**
	 * Use this ctor to store the touch icon in the bookmarks database for the originalUrl so we take account
	 * of redirects. Used when the user bookmarks a page from outside the bookmarks activity.
	 */
	 ...
	 * Used when the user creates a bookmark from within the bookmarks activity and there haven't been any
	 * redirects. TODO: Would be nice to set the user agent here so that there is no potential for the three
	 * different ctors here to return different icons.
	 */
	...
	/**
	 * Use this ctor to not store the touch icon in a database, rather add it to the passed Message's data
	 * bundle with the key {@link BrowserContract.Bookmarks#TOUCH_ICON} and then send the message.
	 */
