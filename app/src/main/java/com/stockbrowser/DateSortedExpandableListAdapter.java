...
/**
 * ExpandableListAdapter which separates data into categories based on date. Used for History and Downloads.
 */
public class DateSortedExpandableListAdapter extends BaseExpandableListAdapter {
	...
	/**
	 * Get the byte array at cursorIndex from the Cursor.  Assumes the Cursor has already been moved to the
	 * correct position.  Along with {@link #getInt} and {@link #getString}, these are provided so the client
	...
	/**
	 * Get the integer at cursorIndex from the Cursor.  Assumes the Cursor has already been moved to the
	 * correct position.  Along with {@link #getBlob} and {@link #getString}, these are provided so the
	 * client
	 * does not need to access the Cursor directly
	 *
	 * @param cursorIndex Index to query the Cursor.
	...
	/**
	 * Get the long at cursorIndex from the Cursor.  Assumes the Cursor has already been moved to the correct
	 * position.
	 */
	...
	/**
	 * Get the String at cursorIndex from the Cursor.  Assumes the Cursor has already been moved to the
	 * correct position.  Along with {@link #getInt} and {@link #getInt}, these are provided so the client
	 * does not need to access the Cursor directly
	 *
	 * @param cursorIndex Index to query the Cursor.
	...
	/**
	 * Translates from a group position in the ExpandableList to a bin.  This is necessary because some
	 * groups
	 * have no history items, so we do not include those in the ExpandableList.
	 *
	 ...
	 * @return boolean False if the cursor is closed, so the Cursor was not moved.  True on success.
	 */
