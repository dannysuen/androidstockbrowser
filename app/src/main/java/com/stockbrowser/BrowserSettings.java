...
import com.stockbrowser.search.SearchEngines;
...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
	...
	/**
	 * Syncs all the settings that have no UI These cannot change, so we only need to set them once per
	 * WebSettings
	 */
	@TargetApi(16)
	...
	public void clearHistory() {
		...
		com.stockbrowser.compats.Browser.clearHistory(resolver);
		com.stockbrowser.compats.Browser.clearSearches(resolver);
	}
	 ...
	 *
	 * @return The last browser start time as System.currentTimeMillis. This can be 0 if this is the first
	 * time or the last tab was closed.
	 */
	 ...
	 *
	 * @param time The last time as System.currentTimeMillis that the browser was started. This should be set
	 *             to 0 if the last tab is closed.
	 */
	...
	/**
	 * Used to determine whether or not the previous browser run crashed. Once the previous state has been
	 * determined, the value will be set to false until a pause is received.
	 *
