...
public class IntentHandler {
			...
			if (Intent.ACTION_VIEW.equals(action) ||
				...
				if (url != null && url.startsWith("http")) {
					final Bundle pairs = intent
							.getBundleExtra(com.stockbrowser.compats.Browser.EXTRA_HEADERS);
	...
	/**
	 * Launches the default web search activity with the query parameters if the given intent's data are
	 * identified as plain search terms and not URLs/shortcuts.
	 *
	...
	/**
	 * Launches the default web search activity with the query parameters if the given url string was
	 * identified as plain search terms and not URL/shortcut.
	 *
		...
		if (controller == null || controller.getTabControl() == null
			...
			new AsyncTask<Void, Void, Void>() {
				...
				protected Void doInBackground(Void... unused) {
					com.stockbrowser.compats.Browser.addSearchUrl(cr, newUrl);
					return null;
				}
			}.execute();
		}
	...
	void onNewIntent(Intent intent) {
		...
		if (Intent.ACTION_VIEW.equals(action)
			...
			if (intent.getBooleanExtra(com.stockbrowser.compats.Browser.EXTRA_CREATE_NEW_TAB, false)
					|| urlData.isPreloaded()) {
			...
			final String appId = intent
					.getStringExtra(com.stockbrowser.compats.Browser.EXTRA_APPLICATION_ID);
	...
	/**
	 * A UrlData class to abstract how the content will be set to WebView. This base class uses loadUrl to
	 * show the content.
	 */
