 ...
 * physical display.  A ScrollView is a {@link FrameLayout}, meaning you should place one child in it
 * containing the entire contents to scroll; this child may itself be a layout manager with a complex
 * hierarchy of objects.  A child that is often used is a {@link LinearLayout} in a vertical orientation,
 * presenting a vertical array of top-level items that the user can scroll through. <p/> <p>The {@link
 * TextView} class also takes care of its own scrolling, so does not require a ScrollView, but using the two
 * together is possible to achieve the effect of a text view within a larger container. <p/> <p>ScrollView
...
public class ScrollerView extends FrameLayout {
	...
	/**
	 * The child to give focus to in the event that a child has requested focus while the layout is dirty.
	 * This prevents the scroll from being wrong if the child has not been laid out before requesting focus.
	 */
	protected View mChildToScrollTo = null;
	/**
	 * True if the user is currently dragging this ScrollView around. This is not the same as 'is being
	 * flinged', which can be checked by mScroller.isFinished() (flinging begins when the user lifts his
	 * finger).
	 */
	 ...
	 * hierarchy
	 * would keep track of this for us.
	 */
	...
	/**
	 * ID of the active pointer. This is used to retain consistency during drags/flings if multiple pointers
	 * are used.
	 */
	private int mActivePointerId = INVALID_POINTER;
	/**
	 * The StrictMode "critical time span" objects to catch animation stutters.  Non-null when a
	 * time-sensitive animation is in-flight.  Must call finish() on them when done animating. These are
	 * no-ops on user builds.
	 */
	// 注释掉隐藏方法
	...
	public ScrollerView(Context context, AttributeSet attrs, int defStyle) {
		...
		// 注释掉
		//        TypedArray a =
		//                context.obtainStyledAttributes(attrs, com.android.internal.R.styleable.ScrollView,
		// defStyle, 0);
	 ...
	 * just as
	 * if the event had been dispatched to it by the view hierarchy.
	 *
					...
					if (!mIsBeingDragged) {
						if (mHorizontal) {
							...
							if (hscroll != 0) {
								...
								int delta = 0;
								try {
									Method m = getClass().getMethod("getHorizontalScrollFactor",
											ScrollerView.class);
									Integer horizontalScrollFactor = (Integer) m.invoke(this);
									delta = (int) (hscroll * horizontalScrollFactor);
								} catch (Exception e) {
							...
							if (vscroll != 0) {
								...
								int delta = 0;
								try {
									Method m = getClass().getMethod("getVerticalScrollFactor", ScrollerView
											.class);
									Integer horizontalScrollFactor = (Integer) m.invoke(this);
									delta = (int) (vscroll * horizontalScrollFactor);
								} catch (Exception e) {
	...
	/**
	 * <p> Finds the next focusable component that fits in this View's bounds (excluding fading edges)
	 * pretending that this View's top is located at the parameter top. </p>
	 *
	 * @param topFocus           look for a candidate at the top of the bounds if topFocus is true, or at the
	 *                           bottom of the bounds if topFocus is false
	 * @param top                the top offset of the bounds in which a focusable must be found (the fading
	 *                           edge is assumed to start at this position)
	 * @param preferredFocusable the View that has highest priority and will be returned if it is within my
	 *                           bounds (null is valid)
	...
	private View findFocusableViewInMyBounds(final boolean topFocus,
		...
		final int bottomWithoutFadingEdge = top + (mHorizontal ? getWidth() : getHeight()) -
				fadingEdgeLength;
	...
	/**
	 * <p>Handles scrolling in response to a "page up/down" shortcut press. This method will scroll the view
	 * by one page up or down and give the focus to the topmost/bottommost component in the new visible area.
	 * If no component is a good candidate for focus, this scrollview reclaims the focus.</p>
	 *
	 * @param direction the scroll direction: {@link android.view.View#FOCUS_UP} to go one page up or {@link
	...
	/**
	 * <p>Handles scrolling in response to a "home/end" shortcut press. This method will scroll the view to
	 * the top or bottom and give the focus to the topmost/bottommost component in the new visible area.
	 * If no
	 * component is a good candidate for focus, this scrollview reclaims the focus.</p>
	 *
	...
	/**
	 * <p>Scrolls the view to make the area defined by <code>top</code> and <code>bottom</code> visible. This
	 * method attempts to give the focus to a component visible in this area. If no component can be focused
	 * in the new visible area, the focus is reclaimed by this ScrollView.</p>
	 *
	...
	/**
	 * If rect is off screen, scroll just enough to get it (or at least the first screen size chunk of it) on
	 * screen.
	 *
	...
	/**
	 * When looking for focus in children of a scroll view, need to be a little more careful not to give
	 * focus
	 * to something that is scrolled off screen. <p/> This is more expensive than the default {@link
	 ...
	 *
	 * @param velocityY The initial velocity in the Y direction. Positive numbers mean that the finger/cursor
	...
	/**
	 * {@inheritDoc} <p/> <p>This version also clamps the scrolling to the bounds of our child.
	 */
	@Override
	...
	/**
	 * Used to indicate that the parent of this view should be invalidated. This functionality is used to
	 * force the parent to rebuild its display list (when hardware-accelerated), which is necessary when
	 * various parent-managed properties of the view change, such as alpha, translationX/Y, scrollX/Y,
	 * scaleX/Y, and rotation/X/Y. This method will propagate an invalidation event to the parent.
	 *
	 * @hide
	 */
	// 从ViewGropu
