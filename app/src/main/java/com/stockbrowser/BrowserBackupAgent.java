...
/**
 * Settings backup agent for the Android browser.  Currently the only thing stored is the set of bookmarks.
 * It's okay if I/O exceptions are thrown out of the agent; the calling code handles it and the backup
 * operation simply fails.
 *
 * @hide
 */
public class BrowserBackupAgent extends BackupAgent {
	...
	/**
	 * Restore from backup -- reads in the flattened bookmark file as supplied from the backup service,
	 * parses
	 * that out, and rebuilds the bookmarks table in the browser database from it.
	 */
	@Override
