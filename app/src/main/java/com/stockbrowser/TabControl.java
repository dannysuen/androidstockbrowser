...
class TabControl {
	...
	/**
	 * Return the current tab's main WebView. This will always return the main WebView for a given tab and
	 * not
	 * a subwindow.
	 *
	 ...
	 *
	 * @return The Tab for the specified position or null if the tab does not exist.
	 */
	 ...
	 * Create a new tab.
	 *
	 * @return The newly createTab or null if we have reached the maximum number of open tabs.
	 */
	...
	/**
	 * Create a new tab with default values for closeOnExit(false), appId(null), url(null), and
	...
	/**
	 * save the tab state: current position position sorted array of tab ids for each tab id, save the tab
	 * state
	 */
	...
	/**
	 * Check if the state can be restored.  If the state can be restored, the current tab id is returned.
	 * This can be passed to restoreState below in order to restore the correct tab.  Otherwise, -1 is
	 * returned and the state cannot be restored.
	 */
	 ...
	 * @param restoreAll           All webviews get restored, not just the current tab (this does not
	 *                                override
	 ...
	 * Creates a new WebView and registers it with the global settings.
	 *
	 * @param privateBrowsing When true, enables private browsing in the new WebView.
	 */
