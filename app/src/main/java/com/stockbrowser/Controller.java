...
public class Controller implements WebViewController, UiController, ActivityController {
	...
	/**
	 * Only meaningful when mOptionsMenuOpen is true.  This variable keeps track of whether the configuration
	 * has changed.  The first onMenuOpened call after a configuration change is simply a reopening of the
	 * same menu (i.e. mIconView did not change).
	 */
	private boolean mConfigChanged;

	/**
	 * Keeps track of whether the options menu is open. This is important in determining whether to show or
	 * hide the title bar overlay
	 */
	private boolean mOptionsMenuOpen;

	/**
	 * Whether or not the options menu is in its bigger, popup menu form. When true, we want the title bar
	 * overlay to be gone. When false, we do not. Only meaningful if mOptionsMenuOpen is true.
	 */
	...
	/**
	 * Share a page, providing the title, url, favicon, and a screenshot.  Uses an {@link Intent} to launch
	 * the Activity chooser.
	 *
	 * @param c          Context used to launch a new Activity.
	 * @param title      Title of the page.  Stored in the Intent with {@link Intent#EXTRA_SUBJECT}
	 * @param url        URL of the page.  Stored in the Intent with {@link Intent#EXTRA_TEXT}
	 * @param favicon    Bitmap of the favicon for the page.  Stored in the Intent with {@link
	 *                   com.stockbrowser.compats.Browser#EXTRA_SHARE_FAVICON}
	...
	/**
	 * Return the desired width for thumbnail screenshots, which are stored in the database, and used on the
	 * bookmarks screen.
	 *
	...
	/**
	 * Return the desired height for thumbnail screenshots, which are stored in the database, and used on the
	 * bookmarks screen.
	 *
	 * @param context Context for finding out the density of the screen.
	...
	//
	////        try {
	////            Method method = view.getClass().getDeclaredMethod("getContentWidth");
	////        }
	//
	//
	...
	private void onPreloginFinished(Bundle icicle, Intent intent, long currentTabId,
		...
		if (currentTabId == -1) {
			...
			if (intent == null) {
				...
				if (t != null) {
					t.setAppId(intent.getStringExtra(com.stockbrowser.compats.Browser.EXTRA_APPLICATION_ID));
				}
				WebView webView = t.getWebView();
				if (extra != null) {
					int scale = extra.getInt(com.stockbrowser.compats.Browser.INITIAL_ZOOM_LEVEL, 0);
	...
	/**
	 * As the menu can be open when loading state changes we must manually update the state of the
	 * stop/reload
	 * menu item
	 */
	 ...
	 * @return boolean True if we successfully switched to a different tab.  If the indexth tab is null,
	 * or if
	 * that tab is the same as the current one, return false.
	 */
	@Override
	...
	/**
	 * Load the URL into the given Tab and update the title bar to reflect the new load.  Call this
	 * instead of
	 * WebView.loadUrl directly.
	 *
	 * @param tab The Tab used to load url.
	...
	/**
	 * Load UrlData into a Tab and update the title bar to reflect the new load.  Call this instead of
	 * UrlData.loadIn directly.
	 *
	...
	private static class Download implements OnMenuItemClickListener {
		...
		/**
		 * Creates a File based on the current time stamp and uses the mime type of the DataUri to get the
		 * extension.
		 */
